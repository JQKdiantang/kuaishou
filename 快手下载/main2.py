...
if __name__ == '__main__':
    ...
    # mySqlite.FillAuthorDataTimeZero();

    idlist = get_ids()
    authors = []
    for data in idlist:
        video = [data, link, pcursor, ck, ua]
        videos = (video, None)
        authors.append(videos)
    pool = threadpool.ThreadPool(3)  # 线程池设置
    tasks = threadpool.makeRequests(save, authors)
